			...
			<van-col span="18" class="btn-area">
				...
				<template v-if="inProgress">
					...
					<template v-if="hasAnswered">
						<van-button class="submit-btn" round type="info" @click="submit">提交</van-button>
					</template>
					
					<template v-else>
						<van-button class="submit-btn" round disabled type="info">提交</van-button>
					</template>
				...
				<template v-else>
					...
					<template v-if="status === 'first'">
						<van-button round class="submit-btn"  type="primary" @click="next">下一题</van-button>
					</template>
					
					<template v-else-if="status === 'last'">
						<van-button round class="question-btn" type="primary" @click="pre">上一题</van-button>
						<van-button round class="question-btn"  type="primary" @click="finish">完成</van-button>
					</template>
					
					<template v-else>
						<van-button round class="question-btn" type="primary" @click="pre">上一题</van-button>
						<van-button round class="question-btn"  type="primary" @click="next">下一题</van-button>
					</template>
