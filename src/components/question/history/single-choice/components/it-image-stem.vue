	...
	<content class="nui-scroll">
		...
		<div class="options">
			<template v-for="(value, key) in questionData.options">
				...
				<template v-else>
					<small class="selected" v-if="selected && key === selected" :key="key" @click="selectAnswer(key)">{{ key }}.{{ value }}</small>
					<small v-else :key="key" @click="selectAnswer(key)">{{ key }}.{{ value }}</small>
				</template>
